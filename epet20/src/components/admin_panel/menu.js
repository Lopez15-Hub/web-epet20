                ...
                <div className="list-group">
                    ...
                    }
                    {
                        role === "secretaria" ? <>
                            <Link to="perfil" className=" list-group-item-action p-2 m-2 admin-item rounded-xl shadow-sm d-flex">   <FaUserAlt className="mr-2 ml-1" /> Mi Perfil</Link>
                            <div>
                                <button onClick={(e) => showEstudiantes(e)} className=" list-group-item-action p-2 m-2 admin-item rounded-xl shadow-sm d-flex">
...

                    {
                        role === "administrador" ? <>
                            ...
                            <div>
                                <button onClick={() => showEstudiantes()} className=" list-group-item-action p-2 m-2 admin-item rounded-xl shadow-sm d-flex">
                                    <MdSchool className="mr-2 ml-1" />  Estudiantes <div className='mx-auto'></div> {
                                        visible ? <FaArrowDown className="mt-1 d-flex   mx-auto" /> : <FaArrowRight className="mt-1 d-flex   mx-auto" />}</button>
                                {
                                    visible ?
                                        <motion.div exit={{ opacity: 0 }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                            <Link to="estudiantes/admin" className="list-group-item-action p-2 m-2 d-flex">Administrar</Link>
                                            <Link to="estudiantes/upload" className="list-group-item-action p-2 m-2 d-flex">Añadir un archivo</Link>

                                        </motion.div> : ''
                                }
                            </div>
                            <div>
                                <button onClick={() => showSecretaria()} className=" list-group-item-action p-2 m-2 admin-item rounded-xl shadow-sm d-flex">
                                    <FaFemale className="mr-2 ml-1" />  Secretaría <div className='mx-auto'></div> {
